...
def test_build_upload_plan_contains_expected_zones():
    ...
    assert any("/raw/sample/" in key for key in s3_keys)
    assert any("/silver/sample/" in key for key in s3_keys)
    assert any("/gold/sample/" in key for key in s3_keys)
    assert any("/reports/sample/" in key for key in s3_keys)
...
def test_build_upload_plan_contains_vendor_payments_files():
    ...
    assert "mart_spending_by_department.csv" in local_files
    assert "mart_spending_by_supplier_top_n.csv" in local_files
    assert "mart_pending_by_department.csv" in local_files
    assert "mart_fund_category_summary.csv" in local_files
